...
# ──────────────────────────────────────────────────────────────────────────
# 魔法物品施法（R-ADD-034）/ 特殊能力施法（R-ADD-035）
# ──────────────────────────────────────────────────────────────────────────

@dataclass
class MagicItemSpellSlot:
    """魔法物品的法术使用计数。"""
    item_id: str
    spell_name: str
    uses_per_day: int
...
def cast_from_magic_item(spell_name: str,
                         item_slot: MagicItemSpellSlot,
                         caster: CasterState,
                         targets: Optional[list] = None,
                         slot_level: int = 0) -> dict:
    """魔法物品施法：无需法术位，消耗物品使用次数。

    规则: R-ADD-034 魔法物品施法（R-SPL 补遗）
          castFromMagicItem(item, spell): casterSlotCost=0;
          item.spellUsesPerDay=specified; onCast: item.uses-=1
    出处: topics/玩家手册2024/法术/法术环阶.htm

    返回: {"success": bool, "slot_consumed": False,
            "item_uses_remaining": int}
    """
    if item_slot.uses_remaining <= 0:
        return {"success": False, "error": "物品使用次数已耗尽",
                "slot_consumed": False}

    # 魔法物品施法不耗法术位
    item_slot.uses_remaining -= 1
    return {
        "success": True,
        "spell": spell_name,
        "slot_consumed": False,
        "item_uses_remaining": item_slot.uses_remaining,
        "source": "magic_item",
    }


def cast_by_special_ability(feature_name: str,
                            spell_name: str,
                            uses_per_day: int,
                            uses_remaining: int,
                            caster: CasterState,
                            targets: Optional[list] = None) -> dict:
    """特殊能力施法：无需法术位，消耗特性每日次数。

    规则: R-ADD-035 特殊能力施法（R-SPL 补遗）
          castBySpecialAbility(feature, spell): casterSlotCost=0;
          feature.usesPerDay=specified; onCast: feature.uses-=1
    出处: topics/玩家手册2024/法术/法术环阶.htm

    返回: {"success": bool, "uses_remaining": int}
    """
    if uses_remaining <= 0:
        return {"success": False, "error": f"特性 {feature_name} 使用次数已耗尽",
                "slot_consumed": False}
    return {
        "success": True,
        "spell": spell_name,
...
def teleport_carried_creatures(spell_name: str,
                               has_exception: bool = False) -> dict:
    """传送时触碰着的生物默认不随传送，除非法术声明例外。

    规则: R-ADD-022 传送触碰生物不随行（补充 R-GLS-038）
          touched_creature_not_transported unless effect_excepts
    出处: topics/玩家手册2024/术语汇编/常见规则词汇.htm

    返回: {"carries_touched": bool}
    """
    return {"carries_touched": has_exception,
            "spell": spell_name}
