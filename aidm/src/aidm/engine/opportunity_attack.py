...
def opportunity_attack_timing() -> str:
    """借机攻击发生于目标离开触及范围前的那一刻（而非离开后）。

    规则: R-ADD-007 借机攻击时机（补充 R-CMB-025）
          timing=resolves_at_moment_before_target_exits_reach
    出处: topics/玩家手册2024/进行游戏/近战攻击.htm

    说明: 这是语义锚点函数。调用方在触发OA时应在离开前那一刻结算，
          保证攻击命中后目标仍未移出触及范围。
    """
    return "before_target_exits_reach"
...
def _self_test() -> None:
    ...
    # ── R-ADD-007: 借机攻击时机 ──
    assert opportunity_attack_timing() == "before_target_exits_reach"
