...
def action_hide(attacker: Combatant, stealth_mod: int, stealth_prof: int,
    ...
    r = check.ability_check(mod=stealth_mod, prof=stealth_prof,
                            ...
                            advantage=advantage, disadvantage=disadvantage)
    if r.success:
        on_hide_success(attacker)                        # R-CMB-009 + R-ADD-013 躲藏成功→隐形
...
# ──────────────────────────────────────────────────────────────────────────
# 躲藏结束触发（R-ADD-012）与躲藏→隐形（R-ADD-013）
# ──────────────────────────────────────────────────────────────────────────

HIDE_END_EVENT = frozenset({
    "noise_above_whisper",   # 发出高于低语的响声
    "found_by_enemy",        # 被敌人找到
    "make_attack_roll",      # 进行攻击检定
    "cast_verbal_spell",     # 施展带言语成分的法术
})


def end_hide_triggers(c: Combatant, event: str) -> bool:
    """检测躲藏是否因某事件结束。

    规则: R-ADD-012 躲藏结束触发
          4种情况结束躲藏：发出高于低语音响/被敌人找到/进行攻击检定/
          施展带言语成分法术
    出处: topics/玩家手册2024/术语汇编/动作.htm

    返回: True 表示躲藏已结束（hidden被清除）。
    """
    if not c.hidden:
        return False
    if event not in HIDE_END_EVENT:
        return False
    c.hidden = False
    return True


def on_hide_success(c: Combatant) -> None:
    """躲藏检定成功时进入隐形状态。

    规则: R-ADD-013 躲藏成功→隐形状态（补充 R-GLS-018/R-CMB-009）
          hide_success → add_condition(invisible)
    出处: topics/玩家手册2024/术语汇编/动作.htm
    说明: 除设置 hidden=True 外，还应令生物处于隐形(invisible)状态。
          实际使用中由 ConditionState.add("隐形") 实现；
          此函数仅设置 Combatant 侧的 hidden 标记。
    """
    c.hidden = True
...
def communicate(actor: Combatant, content: str) -> dict:
    """回合内交流：简短说话/手势免费，长篇说/影响动作需消耗动作。

    规则: R-ADD-001 交流免费（R-CMB 补遗）
          free_communication=true（不耗 action/move）；
          long_speech_or_influence → requires action
    出处: topics/玩家手册2024/进行游戏/战斗流程.htm

    返回: {"free": True/False, "requires_action": True/False}
    """
    # 简短交流（≤50字约等于一两个句子）视为免费
    is_short = len(content) <= 50
    if is_short:
        return {"free": True, "requires_action": False}
    # 长篇或影响他人 → 需要动作
    return {"free": False, "requires_action": True}
...
def influence_branch(creature_willingness: str,
                     creature_int_score: int = 10) -> dict:
    """影响怪物按其意愿分三分支：愿意→直接执行；不愿意→直接拒绝；犹豫→属性检定。

    规则: R-ADD-014 影响动作三分支流程（补充 R-GLS-016）
          willing→no_check,act; unwilling(有悖阵营)→no_check,refuse;
          hesitant→check vs dc=max(15,int_score)
    出处: topics/玩家手册2024/术语汇编/动作.htm

    返回: {"branch": "act"|"refuse"|"check", "dc": int|None}
    """
    w = creature_willingness.lower()
    if w in ("愿意", "willing"):
        return {"branch": "act", "dc": None}
    if w in ("不愿意", "不情愿", "unwilling", "refuse"):
        return {"branch": "refuse", "dc": None}
    if w in ("犹豫", "犹豫不决", "hesitant"):
        return {"branch": "check", "dc": max(15, creature_int_score)}
    # 默认保守：需检定
    return {"branch": "check", "dc": max(15, creature_int_score)}
...
def _self_test() -> None:
    ...
    # ── R-ADD-001: 交流免费 ──
    c_comm = Combatant(cid="cm", name="Commander", speed=30)
    assert communicate(c_comm, "小心！")["free"] is True
    assert communicate(c_comm, "我有一个长达数百字的精妙计划……" + "。" * 100)["requires_action"] is True

    # ── R-ADD-012: 躲藏结束触发 ──
    c_hide = Combatant(cid="hd", name="Hidden", speed=30)
    c_hide.hidden = True
    assert end_hide_triggers(c_hide, "make_attack_roll") is True   # 攻击检定结束躲藏
    assert c_hide.hidden is False
    # 未躲藏时调用无效
    assert end_hide_triggers(c_hide, "noise_above_whisper") is False

    # ── R-ADD-013: 躲藏成功→隐形 ──
    c_hide2 = Combatant(cid="hd2", name="Hidden2", speed=30)
    on_hide_success(c_hide2)
    assert c_hide2.hidden is True

    # ── R-ADD-014: 影响动作三分支 ──
    assert influence_branch("愿意")["branch"] == "act"
    assert influence_branch("不愿意")["branch"] == "refuse"
    r_hes = influence_branch("犹豫", creature_int_score=18)
    assert r_hes["branch"] == "check" and r_hes["dc"] == 18
    r_hes2 = influence_branch("hesitant", creature_int_score=12)
    assert r_hes2["branch"] == "check" and r_hes2["dc"] == 15  # 下限15
