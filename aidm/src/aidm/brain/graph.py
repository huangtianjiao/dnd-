...
def _resolve_ability_check(ch, it) -> dict:
    """技能/属性检定 — 熟练从角色卡持久化数据读取（非LLM猜测）。

    R-CHK-005: 技能熟练检定 = d20 + 属性调整 + (熟练加值 if 熟练)
    R-CHK-010: 属性检定 = d20 + 属性调整 + (熟练加值 if 属性豁免熟练)
    """
    ability = it.get("ability") or "str"
    dc = int(it.get("dc") or 10)
    skill = it.get("skill") or ""
    # 优先从角色卡读取熟练；若角色卡无此技能数据则回退到LLM分类（向后兼容）
    proficient = ch.is_proficient_in_skill(skill) if skill else bool(it.get("proficient"))
    ...
    return {"kind": "ability_check", "check_total": r.total, "d20": r.d20,
            ...
            "skill": skill, "proficient": proficient}
...
def _resolve_social(ch, it) -> dict:
    ...
    R-CON-012/R-DM-047"""
    # 英语→中文技能名映射
    _EN_SKILL_MAP = {"persuasion": "游说", "deception": "欺瞒",
                     "intimidation": "威吓", "performance": "表演",
                     "insight": "洞悉", "investigation": "调查",
                     "perception": "察觉", "medicine": "医药"}
    ...
    cn_skill = _EN_SKILL_MAP.get(skill, skill)
    proficient = ch.is_proficient_in_skill(cn_skill)
    r = check.ability_check(mod=ch.ability_mod(ability), prof=ch.prof(),
    ...
    return {"kind": "social", "skill": skill, "dc": final_dc,
            ...
            "dc_modifier": dc_modifier, "proficient": proficient}
