...
class CharacterSheet:
    ...
    def __init__(self) -> None:
        ...
        self.speed: int = 30
        self.size: str = ""
        self.skill_prof: list[str] = []    # 熟练技能（来自职业+背景）
        self.tool_prof: list[str] = []     # 熟练工具（来自背景）
        # Step 3 属性
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "skill_prof": list(self.skill_prof),
            "tool_prof": list(self.tool_prof),
...
def step2_choose_origin(
    ...
    # 记录背景提供的技能与工具熟练
    bg_skills = bdata["skill_prof"]
    if isinstance(bg_skills, list):
        sheet.skill_prof = list(bg_skills)
    bg_tools = bdata["tool_prof"]
    if isinstance(bg_tools, str):
        sheet.tool_prof = [bg_tools]
    elif isinstance(bg_tools, list):
        sheet.tool_prof = list(bg_tools)
    return sheet
...
def sheet_to_character(sheet: CharacterSheet) -> "models.Character":
    """将五步车卡 CharacterSheet 转换为持久化 Character 模型。

    桥接熟练数据：save_prof / skill_prof / tool_prof / weapon_prof 全部写入 JSON 列，
    确保 graph.py 裁决链路能从角色卡读取熟练（而非 LLM 猜测）。

    规则的实现回填: graph.py::_resolve_ability_check#R-CHK-005
                      graph.py::_resolve_social#R-CON-012
    """
    ...
    ch = models.Character(
        name=sheet.name,
        race=sheet.race,
        char_class=sheet.class_name,
        level=sheet.level,
    )
    # 属性值
    ch.set_abilities({k.lower(): v for k, v in sheet.scores.items()})
    # HP / AC / 速度
    ch.hp_max = sheet.max_hp()
    ch.hp_current = ch.hp_max
    ch.ac = sheet.ac_unarmored()
    ch.speed = sheet.speed
    # 熟练 — 核心：从规则数据写入持久化列
    ch.set_save_prof(sheet.save_prof)           # 豁免熟练（职业提供）
    ch.set_skill_prof(list(sheet.skill_prof))    # 技能熟练（背景+职业选择）
    ch.set_tool_prof(list(sheet.tool_prof))      # 工具熟练（背景提供）
    ch.weapon_prof_json = sheet.weapon_prof      # 武器熟练（职业提供）
    return ch
...
def _self_test() -> None:
    ...
    # ── 熟练桥接测试 ──
    print("[char_create] 熟练桥接测试...")
    from aidm.stats import models as _m
    ch = sheet_to_character(sheet)
    # 验证属性值
    assert ch.ability_mod("str") == 2, f"STR mod应为+2，实际{ch.ability_mod('str')}"  # 15→+2
    assert ch.hp_max == 11, f"HP应为11，实际{ch.hp_max}"
    # 验证熟练数据持久化
    assert ch.save_prof == ["STR", "CON"], f"战士豁免熟练应为[STR,CON]，实际{ch.save_prof}"
    assert "运动" in ch.skill_prof, f"士兵背景应有运动熟练，实际{ch.skill_prof}"
    assert "威吓" in ch.skill_prof, f"士兵背景应有威吓熟练，实际{ch.skill_prof}"
    assert ch.is_proficient_in_save("STR"), "战士应STR豁免熟练"
    assert ch.is_proficient_in_save("CON"), "战士应CON豁免熟练"
    assert not ch.is_proficient_in_save("INT"), "战士不应INT豁免熟练"
    assert ch.is_proficient_in_skill("运动"), "战士+士兵应有运动熟练"
    assert ch.is_proficient_in_skill("威吓"), "战士+士兵应有威吓熟练"
    assert not ch.is_proficient_in_skill("奥秘"), "战士不应有奥秘熟练"
    assert ch.is_proficient_in_weapon("简易"), "战士应有简易武器熟练"
    assert ch.is_proficient_in_weapon("军用"), "战士应有军用武器熟练"
    assert ch.weapon_prof_json == "简易和军用武器"
    # 验证 spell_save_dc / spell_attack_bonus 为 None（战士无施法）
    assert sheet.spell_save_dc_value() is None
    assert sheet.spell_atk_bonus() is None
    # ── 法师熟练桥接测试 ──
    mage_sheet = create_character(
        class_name="法师", race="精灵", background="智者",
        scores_method="standard_array", alignment="绝对中立", name="大法师2",
    )
    mage_ch = sheet_to_character(mage_sheet)
    assert mage_ch.save_prof == ["INT", "WIS"], f"法师豁免熟练应为[INT,WIS]，实际{mage_ch.save_prof}"
    assert "奥秘" in mage_ch.skill_prof, f"智者背景应有奥秘熟练，实际{mage_ch.skill_prof}"
    assert "历史" in mage_ch.skill_prof, f"智者背景应有历史熟练，实际{mage_ch.skill_prof}"
    assert mage_ch.is_proficient_in_save("INT"), "法师应INT豁免熟练"
    assert mage_ch.is_proficient_in_save("WIS"), "法师应WIS豁免熟练"
    print("[char_create] 熟练桥接测试通过 ✓")
