...
def override_pace_for_flyer(can_fly: bool,
                            can_ignore_difficult_terrain: bool,
                            terrain_has_difficult: bool = False) -> str:
    """飞行或无视困难地形的生物，无论地形如何都可用快速步调。

    规则: R-ADD-027 飞行/无视困难地形可快速步调（补充 R-DM-032/033）
          if can_fly_or_ignore_difficult_terrain(actor):
          party_max_pace="fast"
    出处: topics/城主指南2024/2.运作游戏/运作探索/旅行.htm

    返回: 最高可用步调名称（"快速"/"中速"/"慢速"）。
    """
    if can_fly or can_ignore_difficult_terrain:
        return "快速"
    return "快速" if not terrain_has_difficult else "中速"
