...
class Character(SQLModel, table=True):
    ...
    # 熟练（R-CHK-003 豁免熟练 + R-CHK-005 技能熟练 + R-CHK-007 工具熟练）
    skill_prof_json: str = Field(default="[]")     # 熟练技能名列表（中文，如["察觉","运动"]）
    save_prof_json: str = Field(default="[]")      # 熟练豁免属性缩写列表（如["STR","CON"]）
    tool_prof_json: str = Field(default="[]")      # 熟练工具名列表（如["盗贼工具","草药工具"]）
    weapon_prof_json: str = Field(default="")      # 武器熟练字符串（如"简易和军用武器"）
    # 数值
    hp_current: int = 0
    hp_max: int = 0
    temp_hp: int = 0
    ...
    # —— 熟练桥接（R-CHK-003/005/007）——
    # 技能→属性映射（D&D 5E 标准）
    SKILL_TO_ABILITY: ClassVar[dict[str, str]] = {
        "运动": "str", "特技": "dex", "巧手": "dex", "隐匿": "dex",
        "奥秘": "int", "历史": "int", "调查": "int", "自然": "int", "宗教": "int",
        "驯兽": "wis", "洞悉": "wis", "医药": "wis", "察觉": "wis", "求生": "wis",
        "欺瞒": "cha", "威吓": "cha", "表演": "cha", "游说": "cha",
    }

    @property
    def skill_prof(self) -> list[str]:
        """熟练技能名列表（中文）。规则: R-CHK-005"""
        return json.loads(self.skill_prof_json)

    def set_skill_prof(self, skills: list[str]) -> None:
        self.skill_prof_json = json.dumps(skills)

    @property
    def save_prof(self) -> list[str]:
        """熟练豁免属性缩写列表。规则: R-CHK-003"""
        return json.loads(self.save_prof_json)

    def set_save_prof(self, saves: list[str]) -> None:
        self.save_prof_json = json.dumps(saves)

    @property
    def tool_prof(self) -> list[str]:
        """熟练工具名列表。规则: R-CHK-007"""
        return json.loads(self.tool_prof_json)

    def set_tool_prof(self, tools: list[str]) -> None:
        self.tool_prof_json = json.dumps(tools)

    def is_proficient_in_skill(self, skill_name: str) -> bool:
        """检查角色在指定技能上是否熟练。规则: R-CHK-005"""
        if not skill_name:
            return False
        return skill_name in self.skill_prof

    def is_proficient_in_save(self, ability: str) -> bool:
        """检查角色在指定属性豁免上是否熟练。规则: R-CHK-003

        ability: 属性缩写（str/dex/con/int/wis/cha）或大写（STR/DEX/...）。
        """
        if not ability:
            return False
        ab = ability.upper()
        return ab in [s.upper() for s in self.save_prof]

    def is_proficient_in_tool(self, tool_name: str) -> bool:
        """检查角色在指定工具上是否熟练。规则: R-CHK-007"""
        if not tool_name:
            return False
        return tool_name in self.tool_prof

    def is_proficient_in_weapon(self, weapon_category: str) -> bool:
        """检查角色是否熟练指定武器类别（简易/军用/远程/重型等）。

        weapon_category: 类别关键词，如"简易"、"军用"、"远程"、"重型"
        """
        if not weapon_category or not self.weapon_prof_json:
            return False
        wp = self.weapon_prof_json
        # 全武器熟练
        if "所有武器" in wp:
            return True
        # 简易和军用 → 覆盖大部分
        if "简易和军用武器" in wp or "军用和简易武器" in wp:
            return weapon_category in ("简易", "军用")
        if "简易武器" in wp and weapon_category == "简易":
            return True
        if "军用武器" in wp and weapon_category == "军用":
            return True
        return weapon_category in wp
