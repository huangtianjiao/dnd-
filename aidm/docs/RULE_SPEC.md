...
# === R-ADD 审计补遗实现（2026-07-15 新增，12条P0+P1规则）===
# P0 规则 (7条)
aidm/engine/combat.py :: cover_active(source_pos,target_pos,cover_pos,half_size) -> bool  # R-ADD-004 掩护来源侧前置
aidm/engine/combat.py :: move_split(c,segments) -> list[int]  # R-ADD-006 拆分移动跨附赠动作/反应
aidm/engine/opportunity_attack.py :: opportunity_attack_timing() -> str  # R-ADD-007 借机攻击时机(离开触及范围前)
aidm/engine/combat.py :: grapple_capacity(free_hands) -> int + release_grapple(grappler,target) -> bool  # R-ADD-010 擒抱部位限制
aidm/engine/combat.py :: resolve_shove(attacker_str_mod,...,attacker_choice) -> dict  # R-ADD-011 推撞结果(推5尺/倒地)
aidm/engine/actions.py :: end_hide_triggers(c,event) -> bool  # R-ADD-012 躲藏结束触发(响声/被发现/攻击/言语法术)
aidm/engine/actions.py :: on_hide_success(c) -> None  # R-ADD-013 躲藏成功→隐形状态

# P1 规则 (15条)
aidm/engine/actions.py :: communicate(actor,content) -> dict  # R-ADD-001 交流免费(短句不耗动作)
aidm/engine/combat.py :: resume_after_reaction(interrupted) -> None  # R-ADD-005 反应后回合继续
aidm/engine/combat.py :: can_control_mount(trained,accepts) -> bool  # R-ADD-009 受控坐骑受训前置
aidm/engine/actions.py :: influence_branch(willingness,int_score) -> dict  # R-ADD-014 影响动作三分支(愿意/不愿意/犹豫)
aidm/brain/rest.py :: long_rest_interrupt(cause) + restore_max_hp  # R-ADD-015/016 长休打断+HP上限恢复 (已存)
aidm/brain/rest.py :: recharge_features_on_short_rest(character)  # R-ADD-017 短休特性恢复钩子 (已存)
aidm/engine/combat.py :: is_difficult_terrain(tile,creature_size,mover_size) -> bool  # R-ADD-019 困难地形触发清单
aidm/engine/combat.py :: resolve_simultaneous_effects(effects,controller) -> list  # R-ADD-020 同时效应排序
aidm/engine/spellcasting.py :: teleport_carried_creatures(spell,exception) -> dict  # R-ADD-022 传送触碰生物不随行
aidm/engine/combat.py :: grant_initiative_advantage_on_trigger(actor) -> None  # R-ADD-025 引发战斗者先攻优势
aidm/brain/exploration.py :: override_pace_for_flyer(can_fly,ignore_diff) -> str  # R-ADD-027 飞行/无视困难地形可快速步调
aidm/engine/combat.py :: underwater_ranged_full(weapon,distance,normal_range) -> dict  # R-ADD-030 水下远程攻击劣势/自动失手
aidm/engine/combat.py :: flyer_recover_from_fall(fly_speed,prone,falling) -> dict  # R-ADD-031 坠落飞行者半速解除倒地
aidm/data/equipment.py :: ITEM_EFFECTS + get_item_effect(item_id) -> dict  # R-ADD-033 冒险物品效果规则集(19件)
aidm/engine/spellcasting.py :: cast_from_magic_item(spell,item_slot,caster) -> dict + cast_by_special_ability  # R-ADD-034/035 魔法物品施法+特殊能力施法
```

---

*文档版本：v1.4 · 400 条规则点(364原+36审计补遗) · 经7组并行审计复核全部源文 · 修正11处不准确+补遗36条遗漏(P2/P3入待补清单)*
*Phase B/D/G 已由6个子智能体并行实现，15个模块全回归自检通过*
*P0 遗漏项已补齐: 专长系统(74个专长) + 魔法物品系统(30个物品+同调+战利品) + 多人游戏系统(房间管理+多人战斗协调+战利品分配) · 26个模块25通过*
*R-ADD 补齐: P0 7条+P1 15条审计补遗规则已全面实现 · 9/9引擎模块自检通过*
