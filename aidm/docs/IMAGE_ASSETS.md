...
| `aidm/data/images/**` | D&D 主题配图（13 分类） | 680 | 本方案 batch1+batch2 生成 |
...
### 3.2 batch2 生成明细（571 / 571 ✅ 完成）

> 从 5echm_web 抽取条目名（魔法物品 `<H6>` 头、怪物一文件一怪折叠年龄变体、据点特色设施文件名）。
> 旧 key 跑到 ~187 项触达日额度；换 key3 续跑。过程中定位并修复了后台被杀（→ 改同步端点）、日额度（→ 熔断器 + 前台分块/后台同步）。跨两天分批跑完，**全文 571 张全齐，0 失败、0 损坏**。

| 分类 | 计划 | 已成 | 尺寸 | 来源 |
|---|---|---|---|---|
| magic-items 魔法物品 | 339 | **339** ✅ | 1024² | DMG2024 魔法物品详述（按稀有度分文件，`<H6>` 头抽中英名） |
| stronghold 据点地点 | 29 | **29** ✅ | 1536×864 | DMG2024 据点特色设施（文件名即地点名） |
| monsters 核心怪物 | 203 | **203** ✅ | 1024² | 2025 怪物图鉴（折叠年龄变体，按 13 类型均衡各取 ≤16） |

清单：`aidm/data/images/images_manifest_batch2.json`（ok=571, pending=0）。
...
| batch2 | 魔法物品 + 据点地点 + 核心怪物 | 571 | ✅ 完成 |
| batch3（可选） | 全怪物库去重 / 法术图标 / 模组区域图 | ~800–1500 | 待评估 |

> 累计已生成 **680 张**（batch1 109 + batch2 571），0 失败、0 损坏。
