...
## 一、最严重的系统性问题：规则引擎是"孤儿库"

`engine/` 层的规则逻辑（伤害管线、死亡豁免、专注维持、条件优劣势、力竭惩罚、抗性易伤）算法本身大多正确且有自检，但生产路径（`agents/combat_engine.py` → `brain/graph.py`）几乎完全不调用它们：
...
| 长休清空临时HP | `临时生命值.txt` | `long_rest` 不清 temp_hp |
...
| 遗漏项 | 规则书原文出处 | 代码状态 |
|---|---|---|
| 背景属性加成 +2/+1 未应用 | `第三步：确定属性值.htm` | step3 不应用，属性系统性偏低 |
| 起源专长（背景给的1级专长） | `第二步：确定起源.htm` | 存了 `_bg_feat` 但不赋给角色卡 |
| 语言选择 | `第二步：确定起源.htm` | Character 无 languages 字段 |
| 矮人刚毅 +1HP/级 | `矮人.htm` | 未实现，HP 偏低 |
| 野蛮人无甲防御(10+DEX+CON) | `野蛮人.htm` | char_create 只有 10+DEX |
...
| ASI 与专长未强制二选一 | `等级提升.htm` | 可同时获得 |
| 属性值上限20 | `第三步：确定属性值.htm` | `level_up` 不校验 |
| 20级后每30000XP一专长 | `等级提升.htm` | MAX_LEVEL=20，无超20 |
| 旅行步调效应与原文不符 | `进行游戏/旅行.txt:29` | 数值自创 |
| 社交DC修正(±5)/态度转换阈值无原文依据 | `运作游戏/运作交涉/态度.txt` | 引自"报告§7"非规则书 |
| 短休恢复表漏掉野蛮人狂暴 | `野蛮人.htm:85` | rest.py:498-504 |
| rest.py鸭子类型字段与Character不匹配 | `models.py:43-74` vs `rest.py:115-128` | 休息无法落盘 |
...
| 9 | 未知职业HP回退 off-by-one：`(hd+1)//2`（d12→6），应为 `hd//2+1`（d12→7） | `levelup.py:346` | HP少1 |
| 10 | 短休恢复表漏掉野蛮人狂暴 | `rest.py:498-504` | 狂暴不恢复 |
| 11 | rest.py鸭子类型字段与Character不匹配 | `models.py` vs `rest.py:115-128` | 休息无法落盘 |
